...
async def handle_whatsapp_webhook(request: Request):
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️ Agent error, rolling back DB: {e}")
            db.rollback()
            raise
        ...
        # 9️⃣ Send reply back via WhatsApp bridge (handled by bridge itself)
        # success, result = send_message(chat_id, response_text)
        # if not success:
        #     print(f"❌ Failed to send WhatsApp message: {result}")

        print(f"✅ Generated reply for {chat_id}: {response_text[:100]}...")
        print(f"📤 Returning reply to WhatsApp bridge: {response_text}")
